...
class Deployment{
    ...
    async deployJob(processor, jobName,buildFile){
        ...
        for(let k in inventoryJSON){
            if(k =='dashboard')
            {
                ...
                await SSH.executeDroplet(processor, `sudo apt-get -qqq install -y  npm dos2unix`, ip);
                await SSH.executeDroplet(processor, `sudo  -y  npm install forever`, ip);
                await SSH.executeDroplet(processor, `sudo apt-get install redis-server -g`, ip);
            }
            if(k == 'monitor'){
    ...
    async monitorSetup(processor, ip){
        ...
        await SSH.executeDroplet(processor, `sudo  -g npm install forever`, ip);
    }
    ...
    async deployDroplet(processor, jobName, buildFile, ip){
        ...
        for(let i=0; i< jobs.length;i++){
            if(jobs[i].port!=='undefined')
            if(jobs[i].server=='django')
            {
                await SSH.executeVM(processor, `scp -i ~/.ssh/${process.env.vm_name}  -o StrictHostKeyChecking=no  -o UserKnownHostsFile=/dev/null -r ~/django-todolist root@${ip}:~`, ip);
            }
